...
from .services import cancel_call,dashboard_stats,get_company,import_csv,list_companies,log_call,upsert_company
...
class CancelCallInput(BaseModel):
...
def undo_call(call_id:int,payload:CancelCallInput|None=None):
    try: return cancel_call(call_id,payload.reason if payload else None)
    except ValueError as exc: raise HTTPException(400,str(exc))
