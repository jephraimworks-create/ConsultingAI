...
def test_cancel_only_call_returns_company_to_new_queue(client):
    target=client.get("/api/companies?status=NEW").json()[0]
    client.post(f"/api/companies/{target['id']}/calls",json={"reached":"REACHED","opportunity":"YES"})
    company=client.get(f"/api/companies/{target['id']}").json()
    call_id=company["calls"][0]["id"]
    response=client.request("DELETE",f"/api/calls/{call_id}",json={"reason":"Accidental entry"})
    assert response.json()["status"]=="NEW"
    assert client.get(f"/api/companies/{target['id']}").json()["status"]=="NEW"
    assert client.get("/api/stats").json()["labels"]==0

def test_cancel_latest_call_restores_previous_status(client):
    target=client.get("/api/companies?status=NEW").json()[0]
    client.post(f"/api/companies/{target['id']}/calls",json={"reached":"REACHED","opportunity":"UNKNOWN","follow_up_at":"2026-10-01"})
    client.post(f"/api/companies/{target['id']}/calls",json={"reached":"REACHED","opportunity":"NO"})
    company=client.get(f"/api/companies/{target['id']}").json()
    newest_id=company["calls"][0]["id"]
    response=client.request("DELETE",f"/api/calls/{newest_id}",json={"reason":"Wrong company"})
    assert response.json()["status"]=="FOLLOW_UP"
    assert client.get(f"/api/companies/{target['id']}").json()["status"]=="FOLLOW_UP"
