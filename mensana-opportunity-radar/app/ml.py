...
def train():
    ...
    with db() as con:
        rows=con.execute(f"""SELECT {','.join('f.'+x for x in FEATURES)},c.opportunity FROM calls c JOIN features f ON f.company_id=c.company_id WHERE c.opportunity IN ('YES','NO')""").fetchall()
...
def status():
    with db() as con:
        ...
        labels=con.execute("SELECT COUNT(*) FROM calls WHERE opportunity IN ('YES','NO')").fetchone()[0]
