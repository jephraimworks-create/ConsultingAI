...
def get_company(company_id:int):
    with db() as con:
        ...
        result["calls"]=[dict(r) for r in con.execute("SELECT * FROM calls WHERE company_id=? ORDER BY called_at DESC",(company_id,))]
        return result
...
def log_call(company_id:int,payload:dict):
    ...
    with db() as con:
        ...
        con.execute("""INSERT INTO calls(company_id,reached,opportunity,opportunity_type,strength,meeting,follow_up_at,notes,phone_used) VALUES(?,?,?,?,?,?,?,?,?)""",(company_id,reached,opportunity,payload.get("opportunity_type"),payload.get("strength"),1 if payload.get("meeting") else 0,payload.get("follow_up_at"),payload.get("notes"),company["phone"]))
...
def dashboard_stats():
    with db() as con:
        ...
        labels=dict(con.execute("SELECT COUNT(*) labels,SUM(opportunity='YES') positives FROM calls WHERE opportunity IN ('YES','NO')").fetchone())
